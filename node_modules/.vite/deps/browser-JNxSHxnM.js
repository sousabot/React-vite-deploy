...
const double = 2;
const doublePI = Math.PI * double;
const defaultFps = 60;
const generatedTrue = "true";
const generatedFalse = "false";
const canvasTag = "canvas";
const defaultRetryCount = 0;
const squareExp = 2;
const qTreeCapacity = 4;
const defaultRemoveQuantity = 1;
const defaultRatio = 1;
const defaultReduceFactor = 1;
const subdivideCount = 4;
const inverseFactorNumerator = 1;
const rgbMax = 255;
const hMax = 360;
const sMax = 100;
const lMax = 100;
...
const minVelocity = 0;
const defaultTransformValue = 1;
const minimumSize = 0;
const minimumLength = 0;
const zIndexFactorOffset = 1;
const defaultOpacity = 1;
const clickRadius = 1;
const touchEndLengthOffset = 1;
const minCoordinate = 0;
const removeDeleteCount = 1;
const removeMinIndex = 0;
const defaultFpsLimit = 120;
const minFpsLimit = 0;
const canvasFirstIndex = 0;
const loadRandomFactor = 1e4;
const loadMinIndex = 0;
const one = 1;
const none = 0;
const decayOffset = 1;
const tryCountIncrement = 1;
const minRetries = 0;
const rollFactor = 1;
const minZ = 0;
const defaultRadius = 0;
const posOffset = -quarter;
...
const triple = 3;
const sextuple = 6;
const sNormalizedOffset = 1;
const phaseNumerator = 1;
const defaultRgbMin = 0;
const defaultVelocity = 0;
const defaultLoops = 0;
const defaultTime = 0;
...
(function(MoveDirection$1) {
	MoveDirection$1["bottom"] = "bottom";
	MoveDirection$1["bottomLeft"] = "bottom-left";
	MoveDirection$1["bottomRight"] = "bottom-right";
	MoveDirection$1["left"] = "left";
	MoveDirection$1["none"] = "none";
	MoveDirection$1["right"] = "right";
	MoveDirection$1["top"] = "top";
	MoveDirection$1["topLeft"] = "top-left";
	MoveDirection$1["topRight"] = "top-right";
	MoveDirection$1["outside"] = "outside";
	MoveDirection$1["inside"] = "inside";
...
var Vector3d = class Vector3d {
	...
	normalize() {
		...
		if (length != none) this.multTo(inverseFactorNumerator / length);
	}
	rotate(angle) {
...
function getDistances(pointA, pointB) {
	const dx = pointA.x - pointB.x, dy = pointA.y - pointB.y, squareExp$1 = 2;
	return {
		dx,
		dy,
		distance: Math.sqrt(dx ** squareExp$1 + dy ** squareExp$1)
	};
}
...
function getParticleDirectionAngle(direction, position, center) {
	...
	switch (direction) {
		...
		case MoveDirection.right: return empty;
...
var AnimationMode;
(function(AnimationMode$1) {
	AnimationMode$1["auto"] = "auto";
	AnimationMode$1["increase"] = "increase";
	AnimationMode$1["decrease"] = "decrease";
	AnimationMode$1["random"] = "random";
})(AnimationMode || (AnimationMode = {}));

//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/AnimationStatus.js
var AnimationStatus;
(function(AnimationStatus$1) {
	AnimationStatus$1["increasing"] = "increasing";
	AnimationStatus$1["decreasing"] = "decreasing";
})(AnimationStatus || (AnimationStatus = {}));

//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Types/DestroyType.js
var DestroyType;
(function(DestroyType$1) {
	DestroyType$1["none"] = "none";
	DestroyType$1["max"] = "max";
	DestroyType$1["min"] = "min";
})(DestroyType || (DestroyType = {}));

//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Directions/OutModeDirection.js
var OutModeDirection;
(function(OutModeDirection$1) {
	OutModeDirection$1["bottom"] = "bottom";
	OutModeDirection$1["left"] = "left";
	OutModeDirection$1["right"] = "right";
	OutModeDirection$1["top"] = "top";
})(OutModeDirection || (OutModeDirection = {}));

//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Modes/PixelMode.js
var PixelMode;
(function(PixelMode$1) {
	PixelMode$1["precise"] = "precise";
	PixelMode$1["percent"] = "percent";
})(PixelMode || (PixelMode = {}));

//#endregion
//#region node_modules/@tsparticles/engine/browser/Enums/Types/StartValueType.js
var StartValueType;
(function(StartValueType$1) {
	StartValueType$1["max"] = "max";
	StartValueType$1["min"] = "min";
	StartValueType$1["random"] = "random";
...
function divModeExecute(mode, divs, callback) {
	executeOnSingleOrMultiple(divs, (div) => {
		const divMode$1 = div.mode;
		if (div.enable && isInArray(mode, divMode$1)) singleDivModeExecute(div, callback);
	});
}
...
function initParticleNumericAnimationValue(options, pxRatio) {
	...
	if (animationOptions.enable) {
		res.decay = decayOffset$1 - getRangeValue(animationOptions.decay);
		...
		switch (animationOptions.startValue) {
			...
			default:
				...
				if (autoStatus) res.status = getRandom() >= half ? AnimationStatus.increasing : AnimationStatus.decreasing;
				break;
		}
	}
...
function updateAnimation(particle, data, changeDirection, destroyType, delta) {
	const minLoops = 0, minDelay = 0, identity$1 = 1, minVelocity$1 = 0, minDecay = 1;
	if (particle.destroyed || !data || !data.enable || (data.maxLoops ?? minLoops) > minLoops && (data.loops ?? minLoops) > (data.maxLoops ?? minLoops)) return;
	const velocity = (data.velocity ?? minVelocity$1) * delta.factor, minValue = data.min, maxValue = data.max, decay = data.decay ?? minDecay;
	...
	if (data.velocity && decay !== identity$1) data.velocity *= decay;
...
var AlterType;
(function(AlterType$1) {
	AlterType$1["darken"] = "darken";
	AlterType$1["enlighten"] = "enlighten";
})(AlterType || (AlterType = {}));
...
function colorToRgb(engine, input, index, useIndex = true) {
	if (!input) return;
	const color = isString(input) ? { value: input } : input;
	if (isString(color.value)) return color.value === randomColorValue ? getRandomRgbColor() : stringToRgb(engine, color.value);
...
function hslToRgb(hsl) {
	...
	const channel = (temp1$1, temp2$1, temp3) => {
		const temp3Min = 0, temp3Max = 1;
		if (temp3 < temp3Min) temp3++;
		if (temp3 > temp3Max) temp3--;
		if (temp3 * sextuple < temp3Max) return temp1$1 + (temp2$1 - temp1$1) * sextuple * temp3;
		if (temp3 * double < temp3Max) return temp2$1;
		if (temp3 * triple < temp3Max * double) {
			const temp3Offset = double / triple;
			return temp1$1 + (temp2$1 - temp1$1) * (temp3Offset - temp3) * sextuple;
		}
		return temp1$1;
...
function updateColorValue(data, range, decrease, delta) {
	const minLoops = 0, minDelay = 0, identity$1 = 1, minVelocity$1 = 0, minOffset = 0, velocityFactor = 3.6;
	if (!data || !data.enable || (data.maxLoops ?? minLoops) > minLoops && (data.loops ?? minLoops) > (data.maxLoops ?? minLoops)) return;
	if (!data.time) data.time = 0;
	if ((data.delayTime ?? minDelay) > minDelay && data.time < (data.delayTime ?? minDelay)) data.time += delta.value;
	if ((data.delayTime ?? minDelay) > minDelay && data.time < (data.delayTime ?? minDelay)) return;
	const offset = data.offset ? randomInRange(data.offset) : minOffset, velocity = (data.velocity ?? minVelocity$1) * delta.factor + offset * velocityFactor, decay = data.decay ?? identity$1, max = getRangeMax(range), min = getRangeMin(range);
...
function setTransformValue(factor, newFactor, key) {
	...
	if (newValue !== void 0) factor[key] = (factor[key] ?? defaultTransformValue) * newValue;
}
...
(function(InteractivityDetect$1) {
	InteractivityDetect$1["canvas"] = "canvas";
	InteractivityDetect$1["parent"] = "parent";
	InteractivityDetect$1["window"] = "window";
...
var EventListeners = class {
	constructor(container) {
		...
		this._doMouseTouchClick = (e) => {
			...
			if (this._canPush) {
				const mouseInteractivity = container$1.interactivity.mouse, mousePos = mouseInteractivity.position;
				if (!mousePos) return;
			...
			if (e.type === "touchend") setTimeout(() => this._mouseTouchFinish(), touchDelay);
		};
		this._handleThemeChange = (e) => {
			const mediaEvent = e, container$1 = this.container, options = container$1.options, defaultThemes = options.defaultThemes, themeName = mediaEvent.matches ? defaultThemes.dark : defaultThemes.light;
			if (options.themes.find((theme) => theme.name === themeName)?.default.auto) container$1.loadTheme(themeName);
		};
		this._handleVisibilityChange = () => {
			...
			if (document?.hidden) {
				container$1.pageHidden = true;
				container$1.pause();
			} else {
				container$1.pageHidden = false;
				if (container$1.animationStatus) container$1.play(true);
				else container$1.draw(true);
			}
		};
		...
		this._manageInteractivityListeners = (mouseLeaveTmpEvent, add) => {
			const handlers = this._handlers, container$1 = this.container, options = container$1.actualOptions, interactivityEl = container$1.interactivity.element;
			if (!interactivityEl) return;
			const html = interactivityEl, canvas = container$1.canvas;
		...
		this._manageListeners = (add) => {
			const handlers = this._handlers, container$1 = this.container, detectType = container$1.actualOptions.interactivity.detectsOn, canvasEl = container$1.canvas.element;
			let mouseLeaveTmpEvent = mouseLeaveEvent;
			if (detectType === InteractivityDetect.window) {
				container$1.interactivity.element = window;
				mouseLeaveTmpEvent = mouseOutEvent;
			} else if (detectType === InteractivityDetect.parent && canvasEl) container$1.interactivity.element = canvasEl.parentElement ?? canvasEl.parentNode;
			else container$1.interactivity.element = canvasEl;
		...
		this._manageResize = (add) => {
			const handlers = this._handlers, container$1 = this.container;
			if (!container$1.actualOptions.interactivity.events.resize) return;
			if (typeof ResizeObserver === "undefined") {
				manageListener(window, resizeEvent, handlers.resize, add);
				return;
			}
			const canvasEl = container$1.canvas.element;
		...
		this._mouseTouchClick = (e) => {
			const container$1 = this.container, options = container$1.actualOptions, { mouse } = container$1.interactivity;
			mouse.inside = true;
			let handled = false;
			const mousePosition = mouse.position;
			if (!mousePosition || !options.interactivity.events.onClick.enable) return;
			for (const plugin of container$1.plugins.values()) {
		...
		this._mouseTouchMove = (e) => {
			const container$1 = this.container, options = container$1.actualOptions, interactivity = container$1.interactivity, canvasEl = container$1.canvas.element;
			...
			const pxRatio = container$1.retina.pixelRatio;
			if (pos) {
				pos.x *= pxRatio;
				pos.y *= pxRatio;
			}
...
(function(EventType$1) {
	EventType$1["configAdded"] = "configAdded";
	EventType$1["containerInit"] = "containerInit";
	EventType$1["particlesSetup"] = "particlesSetup";
	EventType$1["containerStarted"] = "containerStarted";
	EventType$1["containerStopped"] = "containerStopped";
	EventType$1["containerDestroyed"] = "containerDestroyed";
	EventType$1["containerPaused"] = "containerPaused";
	EventType$1["containerPlay"] = "containerPlay";
	EventType$1["containerBuilt"] = "containerBuilt";
	EventType$1["particleAdded"] = "particleAdded";
	EventType$1["particleDestroyed"] = "particleDestroyed";
	EventType$1["particleRemoved"] = "particleRemoved";
})(EventType || (EventType = {}));
...
var DivType;
(function(DivType$1) {
	DivType$1["circle"] = "circle";
	DivType$1["rectangle"] = "rectangle";
})(DivType || (DivType = {}));
...
var ResponsiveMode;
(function(ResponsiveMode$1) {
	ResponsiveMode$1["screen"] = "screen";
	ResponsiveMode$1["canvas"] = "canvas";
...
var CollisionMode;
(function(CollisionMode$1) {
	CollisionMode$1["absorb"] = "absorb";
	CollisionMode$1["bounce"] = "bounce";
	CollisionMode$1["destroy"] = "destroy";
...
var OutMode;
(function(OutMode$1) {
	OutMode$1["bounce"] = "bounce";
	OutMode$1["none"] = "none";
	OutMode$1["out"] = "out";
	OutMode$1["destroy"] = "destroy";
	OutMode$1["split"] = "split";
})(OutMode || (OutMode = {}));
...
var LimitMode;
(function(LimitMode$1) {
	LimitMode$1["delete"] = "delete";
	LimitMode$1["wait"] = "wait";
})(LimitMode || (LimitMode = {}));
...
var InteractorType;
(function(InteractorType$1) {
	InteractorType$1["external"] = "external";
	InteractorType$1["particles"] = "particles";
...
var ParticleOutType;
(function(ParticleOutType$1) {
	ParticleOutType$1["normal"] = "normal";
	ParticleOutType$1["inside"] = "inside";
	ParticleOutType$1["outside"] = "outside";
...
var Particle = class {
	constructor(engine, container) {
		this.container = container;
		this._calcPosition = (container$1, position, zIndex, tryCount = defaultRetryCount) => {
			for (const plugin of container$1.plugins.values()) {
			...
			const canvasSize = container$1.canvas.size, exactPosition = calcExactPositionOrRandomFromSize({
				size: canvasSize,
				position
				...
				fixOutMode({
					...
					coord: pos.x,
					maxCoord: container$1.canvas.size.width,
				...
				fixOutMode({
					...
					checkModes: [OutMode.bounce],
					coord: pos.y,
					maxCoord: container$1.canvas.size.height,
			...
			if (this._checkOverlap(pos, tryCount)) return this._calcPosition(container$1, void 0, zIndex, tryCount + tryCountIncrement);
			return pos;
		};
		...
		this._checkOverlap = (pos, tryCount = defaultRetryCount) => {
			...
			if (retries >= minRetries && tryCount > retries) throw new Error(`${errorPrefix} particle is overlapping and can't be placed`);
			return !!this.container.particles.find((particle) => getDistance(pos, particle.position) < radius + particle.getRadius());
		};
		this._getRollColor = (color) => {
			if (!color || !this.roll || !this.backColor && !this.roll.alter) return color;
			const backFactor = this.roll.horizontal && this.roll.vertical ? double * rollFactor : rollFactor, backSum = this.roll.horizontal ? Math.PI * half : none;
			if (!(Math.floor(((this.roll.angle ?? none) + backSum) / (Math.PI / backFactor)) % double)) return color;
		...
		this._initPosition = (position) => {
			const container$1 = this.container, zIndexValue = getRangeValue(this.options.zIndex.value);
			this.position = this._calcPosition(container$1, position, clamp(zIndexValue, minZ, container$1.zLayers));
			this.initialPosition = this.position.copy();
			const canvasSize = container$1.canvas.size;
			this.moveCenter = {
...
var RangeType;
(function(RangeType$1) {
	RangeType$1["circle"] = "circle";
	RangeType$1["rectangle"] = "rectangle";
})(RangeType || (RangeType = {}));
...
var Circle = class Circle extends BaseRange {
	...
	intersects(range) {
		...
		if (range instanceof Circle || range.type === RangeType.circle) return r + range.radius > Math.sqrt(distPos.x ** squareExp + distPos.y ** squareExp);
		else if (range instanceof Rectangle || range.type === RangeType.rectangle) {
			const { width, height } = range.size;
			return Math.pow(distPos.x - width, squareExp) + Math.pow(distPos.y - height, squareExp) <= r ** squareExp || distPos.x <= r + width && distPos.y <= r + height || distPos.x <= width || distPos.y <= height;
		}
		return false;
	}
};
...
var QuadTree = class QuadTree {
	constructor(rectangle, capacity) {
		...
		this._subdivide = () => {
			const { x, y } = this.rectangle.position, { width, height } = this.rectangle.size, { capacity: capacity$1 } = this;
			for (let i = 0; i < subdivideCount; i++) {
				const fixedIndex = i % double;
				this._subs.push(new QuadTree(new Rectangle(x + width * half * fixedIndex, y + height * half * (Math.round(i * half) - fixedIndex), width * half, height * half), capacity$1));
			}
			this._divided = true;
		};
		this._points = [];
...
var Particles = class {
	constructor(engine, container) {
		...
		this._initDensityFactor = (densityOptions) => {
			const container$1 = this._container;
			if (!container$1.canvas.element || !densityOptions.enable) return defaultDensityFactor;
			const canvas = container$1.canvas.element, pxRatio = container$1.retina.pixelRatio;
...
var Container = class {
	...
	destroy(remove = true) {
		...
		if (remove) {
			...
			if (idx >= removeMinIndex) mainArr.splice(idx, removeDeleteCount);
		}
...
var getCanvasFromContainer = (domContainer) => {
	...
	if (domContainer instanceof HTMLCanvasElement || domContainer.tagName.toLowerCase() === canvasTag) {
		...
		if (!canvasEl.dataset[generatedAttribute]) canvasEl.dataset[generatedAttribute] = generatedFalse;
	} else {
...
var Engine = class {
	...
	async load(params) {
		const id = params.id ?? params.element?.id ?? `tsparticles${Math.floor(getRandom() * loadRandomFactor)}`, { index, url } = params, currentOptions = itemFromSingleOrMultiple(url ? await getDataFromUrl({
		...
		if (oldIndex >= loadMinIndex) {
			const old = this.item(oldIndex), deleteCount$1 = old ? one : none;
			if (old && !old.destroyed) old.destroy(false);
			items.splice(oldIndex, deleteCount$1, newItem);
